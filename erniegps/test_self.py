...
def test_advantage_split_slow_end():
    """
    Test detecting a negative split when a slow point is at the end of a track
    ...
    # Add one point with the same distance as between each previous point
    # and only taking 45 minutes instead of an hour (to trigger being a negative split)
    #
    trackpoints.append({
        "distancemiles": point_count * 10,
...
def test_advantage_split_slower_end():
    """
    Test detecting a negative split when a few slow points are at the end of a track

    Constant track base + one fast point + several slow point
    ...
    for i in range(0, 5):
        # Add five points with the same distance as between each previous point
        # and taking two hours instead of one
        #
        endtime = starttime + datetime.timedelta(hours=point_count - 1 + (2 * (1 + i)), minutes=45)
        trackpoints.append({
            "distancemiles": (point_count + 1 + i) * 10,
            "elapsedtime": "{0}:45:00".format(point_count - 1 + (2 * (1 + i))),
            "time": str(endtime)
        })
