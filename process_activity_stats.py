...
def main():
    ...
    parser.add_argument('--debug', default=False, help='Print debugging info to stderr', action='store_true')
    ...
    for basename in os.listdir(ACTIVITY_DIR):
        ...
        if "tcx" in filename and (args.filter is None or args.filter.lower() in filename.lower()):
            ...
            logging.debug("activity: {0}".format(activity))
            if activity is None:
            ...
            if create_activity:
                logging.debug("Reading filename: {0}".format(filename))
                activity = erniegps.read_activity(filename)
                logging.debug("Processing activity: {0}".format(activity))
                activity = erniegps.process_activity(activity)

                activity['activity_version'] = ACTIVITY_VERSION

                logging.debug("Created activity ( {0} )".format(create_activity_reason))
