#!/usr/bin/env node
// Format aws ec2 describe-vpcs output into csv data - name, id, etc...
//
// Example usage:
// aws ec2 describe-vpcs | ./format_vpc_json.js
//
...
//
var output_fields = [ "VpcId", "State", "CidrBlock", "IsDefault" ]
...
process.stdin.on('end', function() {
  ...
  incoming_json.Vpcs.forEach(function(vpc) {

        var vpc_tags = {};
        if(!vpc.Tags) { vpc.Tags = [] }

        vpc.Tags.forEach(function(tag) {
          output_tags.forEach(function(output_tag) {
            output_tag = output_tag.toLowerCase();
            if(tag.Key.toLowerCase() === output_tag) {
              vpc_tags[output_tag] = tag.Value;
            }
          });
        });
    ...
    output_fields.forEach(function(output_field) {
      if(!vpc[output_field]) { vpc[output_field] = ''; }
      if(printed_field) {
        stdout_chunks[stdout_chunks.length] = ","
      }
      stdout_chunks[stdout_chunks.length] = vpc[output_field]
      printed_field = true;
    });

    output_tags.forEach(function(output_tag) {
      output_tag = output_tag.toLowerCase();
      if(!vpc_tags[output_tag]) { vpc_tags[output_tag] = ''; }
      stdout_chunks[stdout_chunks.length] = ","
      stdout_chunks[stdout_chunks.length] = vpc_tags[output_tag]
    });
