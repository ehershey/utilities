...
do
  ...
  (echo day,distance,notes,level; cache_get 86400  "http://nikerunning.nike.com/nikeplus/v2/schedules/plus/marathon_${level}_en_us.xml" | sed 's/<\/Day>/#/g' | tr -d \\n | tr \# \\n | sed 's/ *<Day> */0/g' | sed "s/.*PlannedDistance value='//g" | sed 's/<Activity.*//' | sed "s/'.*='/#/g" | sed "s/'.*//g" | tr -d \\t | tr \# ,  | sed 's/\([0-9]\)$/\1,/' | grep -v ^\< | sed s/$/,$level/ | grep -n  '' | tr : ,    )|mongoimport --db "$DB" --collection "$COLLECTION" --type csv --headerline --upsertFields day,level
done
